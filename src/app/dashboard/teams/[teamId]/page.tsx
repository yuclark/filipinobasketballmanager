...
export default function CPUTeamRosterPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  const loadTeamData = async () => {
    try {
      ...
      if (!rosterData) {
        setError("Opposing franchise not found.");
      } else {
        setTeam(rosterData.team as Team);
      ...
      console.error(err);
      setError("Failed to retrieve franchise details.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (error || !team) {
    return (
      <div className="text-center py-12 text-zinc-500">
        <p className="mb-4">{error || "Franchise not found."}</p>
        <Link
